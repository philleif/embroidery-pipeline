...
class WordmarkTracer:
    ...
    def _build_edges(self):
        ...
        while changed:
            ...
            for pt, ends in endpoint_map.items():
                if len(ends) < 2:
                    continue
                best, best_dot = None, 0.55    # only genuinely collinear joins
    ...
    def _build_outline(self, polylines):
        ...
        assigned = np.where(self.ink, lbl[iy, ix], 0)
        ...
        for k, (pts, w, method, rails, extra) in enumerate(polylines):
            ...
            # A hole inside ONE branch is deckle noise, never a counter
            # (counters live between branches) — and a fill with a hole is a
            # shape fill_to_satin silently refuses to convert.
            ...
            def inside(py, px_, _sub=sub, _r0=r0, _c0=c0):
        ...
        for e in edges:
            ...
            for piece, hairline, tstart, tend in self._split_by_width(e):
                ...
                if (L < 1.15 * w and not hairline) or L < 0.5:
                    n_stub += 1
                    continue
                extra = {"piece": piece} if outline else None
                if hairline:
